...
module.exports = function (cli, config) {
    ...
    Chrome(function (chrome) {
        ...
        .flatMap(function (tasks) {
            return downloadItemsAndWrite(tasks, config.getIn(['output', 'dir'])).map(function (files) {
                return {
                    files: files,
                    tasks: tasks
                };
         ...
         */
        var afterPageLoad = requestStream().skipUntil(obs.Page.loadEventFired).takeUntil(afterPageLoadTimeout()).reduce(function (all, item) {
            return all.concat(item);
        }, []);
