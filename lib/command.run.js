...
module.exports = function (cli, config) {
    ...
    Chrome(function (chrome) {
        ...
        function createDownloaderByType(type, fn) {
            return netreq().takeUntil(obs.Page.loadEventFired).filter(function (x) {
                return isType(x, type, config);
         ...
         * given in config
         */
        var home = netreq().takeUntil(obs.Page.loadEventFired).filter(function (x) {
            return x.request.url === x.documentURL;
        });
         ...
         */
        var tasks = Rx.Observable.merge(textRequests, binaryRequests).reduce(function (all, item) {
            return all.concat(item);
        }, []);
        ...
        .flatMap(function (obj) {
            return downloadTextItem(obj.home, conf).map(getReturnObj(obj));
        });
        ...
        var writer = initial.subscribe(function (x) {
