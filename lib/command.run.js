...
const pubPath = x => path.join(__dirname, '../', 'public', x);
...
module.exports = function (cli) {
    ...
    Chrome(function (chrome) {
        ...
        const reqs = obs.Network.requestWillBeSent
            ...
            .map((req, i) => Right(req.url.pathname)
                .map(pathname => path.parse(pathname))
                .chain(parsed =>
                    Right(pubPath(parsed.dir.slice(1)))
                        .map(dirname => ({
                            dirname,
            ...
            .flatMap(x =>
                fromPromise(download(x.req.request.url, x.dirname))
                    .map(dl => x)
            );
                ...
                .flatMap(xs =>
                    ...
                    merge(
                        ...
                        fromPromise(download(target.href)) // write html
                            ...
                            .do(x => tryWrite('public/index.html', x)
