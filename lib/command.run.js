...
module.exports = function (cli, config) {
    ...
    Chrome(function (chrome) {
        ...
        .filter(function (x) {
            ...
            return utils.transform(x, config.get('transforms'));
        })
        /**
        ...
        .flatMap(function (tasks) {
            return downloadItemsAndWrite({ items: tasks, config: config }).map(function (files) {
                return {
                    files: files,
                    tasks: tasks
                };
         ...
         */
        var afterPageLoad = requestStream().skipUntil(obs.Page.loadEventFired).takeUntil(afterPageLoadTimeout()).filter(function (x) {
            return utils.isValidType(x, config.get('whitelist'));
        }).map(function (x) {
            return utils.transform(x, config.get('transforms'));
        }).toArray()
        ...
        }).subscribe(function (x) {}, function (err) {
