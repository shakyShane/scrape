    ...
    Chrome(function (chrome) {
        ...
        function afterPageLoadTimeout() {
            return obs.Page.loadEventFired.concat(Rx.Observable.defer(function () {
                return Rx.Observable.timer(config.afterPageLoadTimeout);
            })).skip(1);
        }
         ...
         */
        .filter(function (x) {
            return utils.isValidType(x, config.whitelist);
        }).filter(function (x) {
            return !utils.isExcludedHost(x, config.hostBlacklist);
        })
        /**
        ...
        ['do'](function (x) {
            ...
            return console.log('=== Waiting for a further ' + config.afterPageLoadTimeout + 'ms before exiting ===');
        })
        /**
        ...
        .flatMap(function (tasks) {
            return downloadItemsAndWrite(tasks, config.output.dir).map(function (files) {
                return {
                    files: files,
                    tasks: tasks
                };
         ...
         */
        .flatMap(function (x) {
            return writeFile(resolve(config.prefix, 'index.html'), x.home.rewritten).map(function (done) {
                return x;
            });
         ...
         */
        requestStream().skipUntil(obs.Page.loadEventFired).takeUntil(afterPageLoadTimeout()).reduce(function (all, item) {
            return all.concat(item);
        }, []).subscribe(function (x) {
